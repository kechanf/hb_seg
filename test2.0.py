...
if(uuid.UUID(int=uuid.getnode()).hex[-12:] == "bc6ee23a04f7"):
    ...
    v3d_path = r"/home/kaifeng/Vaa3D/vaa3d"
...
pred_folder_path = os.path.join(pred_path, "pred")
...
def get_soma_regions_folder(tif_folder, soma_folder, muti_soma_marker_folder, time_out=60):
    ...
    file_names = [f for f in os.listdir(tif_folder) if f.endswith('.tif')]
    partial_func = partial(get_soma_regions_file, tif_folder=tif_folder, soma_folder=soma_folder, muti_soma_marker_folder=muti_soma_marker_folder)
    with Pool(pool_num) as p:
        for _ in tqdm(p.imap(partial_func, file_names),
    ...
    if(muti_soma_marker_path and os.path.exists(muti_soma_marker_path)):
        ...
        with open(muti_soma_marker_path, 'r') as f:
            ...
            if(marker_content.split('\n', 1)[0][0] == '#'):
                marker_content = marker_content.split('\n', 1)[1]
            marker_content = marker_content.split(',')
            label_soma_x, label_soma_y, label_soma_z = float(marker_content[0]), float(marker_content[1]), float(marker_content[2])
            label_soma_x, label_soma_y, label_soma_z = int(label_soma_x / 2), int(label_soma_y / 2), int(label_soma_z / 2)
            ...
            for obj_id in range(1, num_objects + 1):
                ...
                dist_to_label = (soma_x - label_soma_x) ** 2 + (soma_y - label_soma_y) ** 2 + (soma_z - label_soma_z) ** 2
...
def get_skelwithsoma_folder(skel_folder, soma_folder, skelwithsoma_folder):
    ...
    partial_func = partial(get_skelwithsoma_file, skel_folder=skel_folder, soma_folder=soma_folder, skelwithsoma_folder=skelwithsoma_folder)
    with Pool(pool_num) as p:
        for _ in tqdm(p.imap(partial_func, file_names),
...
def connect_to_soma_folder(swc_folder, soma_folder, conn_folder):
    file_names = [f for f in os.listdir(swc_folder) if f.endswith('.swc')]
    partial_func = partial(connect_to_soma_file, swc_folder=swc_folder, soma_folder=soma_folder, conn_folder=conn_folder)
    with Pool(pool_num) as p:
        for _ in tqdm(p.imap(partial_func, file_names),
...
def postprocessing():
    ...
    # connect_to_soma_folder(gcutswc_folder_path, soma_folder_path, conn_folder_path)
...
def rename_muti_soma_markers(muti_soma_marker_folder):
    ...
    for file_name in file_names:
        if("marker" in file_name):
            os.rename(os.path.join(muti_soma_marker_folder, file_name), os.path.join(muti_soma_marker_folder, file_name.replace('v3draw.marker', 'marker')))
