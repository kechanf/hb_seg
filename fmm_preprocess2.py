...
def find_random_paths(predecessor_image, num_paths=5):
    ...
    return paths
def find_path_to_source(predecessor_image, start_point, source=None):
    path = [tuple(start_point)]  # 初始化路径，起始点加入路径

    current_point = np.array(start_point, dtype=int)  # 确保为整数类型数组

    while True:
        # print(f"type of current_point: {type(current_point)} {type(current_point[0])}")
        # print(f"current_point: {current_point}, predecessor_image shape: {predecessor_image.shape}")
        pred_code = predecessor_image[tuple(current_point)]
        if(source is not None):
            if(tuple(current_point) == source):
                break
        if pred_code == -1:  # 到达源点
            break
        # 解码前驱点的坐标偏移
        ...
        # 计算前驱点坐标
        current_point = current_point - np.array([dx, dy, dz])
        current_point = current_point.astype(int)
        # 将前驱点加入路径
        path.append(tuple(current_point))
    # print(path)
    return path
