...
test_source = r"/PBshare/SEU-ALLEN/Projects/Human_Neurons/all_human_cells/all_human_cells_v3draw_8bit"

dir_root = r"/PBshare/SEU-ALLEN/Users/KaifengChen/human_brain/10847_auto_v1.4_12498-13144"
# dir_root = "/data/kfchen/10847"
list_10847 = dir_root + r"/list_10847.xlsx"
list_traced = dir_root + r"/list_traced.xlsx"
swc_root = dir_root + r"/v3dswc"
...
def get_mip(image, projection_direction='xy'):
    ...
    mip = np.max(image, axis=projection_axes)
    mip = augment_gamma(mip)  # gamma correction
    mip = mip.astype("uint8")
    # print(np.max(mip), np.min(mip))
    return mip

def get_mip_swc(swc_file, image, projection_direction='xy', ignore_background=False):
    if projection_direction == 'xy':
        projection_axes = 0
    elif projection_direction == 'xz':
        projection_axes = 1
    ...
    if (projection_axes == 0):
        ...
        cv2.circle(background, (int(point_l.p[1].x), int(point_l.p[1].z)), thickness, color, -1)
    elif (projection_axes == 2):
        cv2.circle(background, (int(point_l.p[1].y), int(point_l.p[1].z)), thickness, color, -1)
...
def concat_images():
    ...
    dir_list = ["v1.3mutisoma_replace_v1.3", "v1.4_12k"]
    folder_path = [folder_path_prefix + dir + "/mip_swc" for dir in dir_list]

    # 新文件夹路径
    concat_folder = r"/PBshare/SEU-ALLEN/Users/KaifengChen/human_brain/concat_mip_1.3&1.4"
    ...
    for i in range(len(folder_path)):
        file_list.append(os.listdir(folder_path[i]))
    # 去除重复
