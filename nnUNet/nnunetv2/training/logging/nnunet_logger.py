...
class nnUNetLogger(object):
    ...
    def plot_progress_png(self, output_folder):
        ...
        fig, ax_all = plt.subplots(6, 1, figsize=(30, 90))  # Update: Now 6 subplots instead of 4

        # Regular progress plot as used in previous versions
        ax = ax_all[0]
        ax2 = ax.twinx()
        ...
        # Subplot for epoch durations
        ax = ax_all[1]
        ax.plot(x_values, [i - j for i, j in zip(self.my_fantastic_logging['epoch_end_timestamps'][:epoch + 1],
                                                 self.my_fantastic_logging['epoch_start_timestamps'])][:epoch + 1], color='b', ls='-', label="epoch duration", linewidth=4)
        ax.set_ylim(0, max(ax.get_ylim()[1], 1))
        ax.set_xlabel("epoch")
        ax.set_ylabel("time [s]")
        ax.legend(loc=(0, 1))

        # Subplot for learning rate
        ax = ax_all[2]
        ...
        # New subplot for train_ce and val_ce
        ax = ax_all[4]
        ax.plot(x_values, self.my_fantastic_logging['train_ce'][:epoch + 1], color='b', ls='-', label="train_ce", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_ce'][:epoch + 1], color='r', ls='-', label="val_ce", linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("cross-entropy")
        ax.legend(loc=(0, 1))

        # New subplot for train_dice and val_dice
        ax = ax_all[5]
        ax.plot(x_values, self.my_fantastic_logging['train_dice'][:epoch + 1], color='b', ls='-', label="train_dice", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_dice'][:epoch + 1], color='r', ls='-', label="val_dice", linewidth=4)
        ax.set_xlabel("epoch")
    ...
    def load_checkpoint(self, checkpoint: dict):
        ...
        if("train_ptls" not in self.my_fantastic_logging.keys()):
            ...
            self.my_fantastic_logging["val_dice"] = [-1] * pre_train_epochs
