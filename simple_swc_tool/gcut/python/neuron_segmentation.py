...
import sys
import time
# from gcut.preprocess import HandyMan
# from gcut.gcut import GCut
from gcut.python.gcut.preprocess import HandyMan
from gcut.python.gcut.gcut import GCut
...
class NeuronSegmentation:
    def __init__(self, swc_path, soma_file_path, scale_z=1, scale_ouput_z=False):
        ...
        self.handy_man = HandyMan(swc_path, soma_file_path, scale_z=self.scale_z)
        self.handy_man.process()
        self.tree_nodes, self.soma_ids = self.handy_man.results()
    ...
    def save(self, out_dir):
        ...
        for soma_id, neuron_tree in neurons.items():
            swc_soma_name = os.path.basename(self.handy_man.swc_path).replace(
                '.swc', '_soma={}.swc'.format(soma_id))
...
def main():
    if len(sys.argv) < 3:
        raise ValueError('Usage: python3 neuron_segmentation.py [swc_path] [soma_file_path]')
    swc_path = sys.argv[1]
    ...
    start = time.time()
    segmentor = NeuronSegmentation(swc_path, soma_file_path, scale_z=5, scale_ouput_z=scale_ouput_z)
    segmentor.segment()
